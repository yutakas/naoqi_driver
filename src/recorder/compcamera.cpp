...
CompressedCameraRecorder::CompressedCameraRecorder( const std::string& topic_, float buffer_frequency ):
...
void CompressedCameraRecorder::write(std_msgs::UInt8MultiArray& img)
{
  gr_->write(topic_img_, img);
}

void CompressedCameraRecorder::writeDump(const ros::Time& time)
{
  boost::mutex::scoped_lock lock_write_buffer( mutex_ );
  boost::circular_buffer< std_msgs::UInt8MultiArray >::iterator it;
...
void CompressedCameraRecorder::reset(boost::shared_ptr<GlobalRecorder> gr, float conv_frequency)
{
  gr_ = gr;
...
void CompressedCameraRecorder::bufferize( std_msgs::UInt8MultiArray& img)
{
  ...
  {
    counter_ = 1;
    buffer_.push_back(img);
  }
}

void CompressedCameraRecorder::setBufferDuration(float duration)
{
  boost::mutex::scoped_lock lock_bufferize( mutex_ );
